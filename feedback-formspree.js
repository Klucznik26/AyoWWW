const AYO_FORMSPREE_ENDPOINT = "https://formspree.io/f/mkjwaqor";

const AYO_DELIVERY_STATUS = {
...
function ayoDeliveryStatus(code = currentLanguageCode) {
    return AYO_DELIVERY_STATUS[code] || AYO_DELIVERY_STATUS.en;
}

const ayoOriginalSetFeedbackLanguage = setFeedbackLanguage;
setFeedbackLanguage = function() {
    ayoOriginalSetFeedbackLanguage();
    const note = document.getElementById("feedback-note");
    const submit = document.getElementById("feedback-submit");
    if (note) note.hidden = true;
    if (submit) submit.textContent = feedbackTranslation().button;
};
...
submitFeedback = async function(event) {
    ...
    const delivery = ayoDeliveryStatus();
    ...
    try {
        const response = await fetch(AYO_FORMSPREE_ENDPOINT, {
            method: "POST",
            body: data,
        ...
        submit.textContent = feedbackTranslation().button;
    }
};
